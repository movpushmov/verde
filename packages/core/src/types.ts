...
export interface StoreWritable<Value> extends Store<Value> {
  ...
  scopeBind(forceScope: Scope): StoreWritable<Value>;
}

export type EventHandler<Payload> = (scope: Scope, payload: Payload) => void;
...
export interface EventCallable<Payload> extends Event<Payload> {
  ...
  scopeBind(forceScope: Scope): EventCallable<Payload>;
}
...
export interface Action<Params, Done, Fail = Error> extends Event<Params> {
  ...
  scopeBind(forceScope: Scope): Action<Params, Done, Fail>;
}
...
export type Model<State extends ModelState> = () => State;
...
export type ModelState = Record<string, Unit>;
